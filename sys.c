...
sem_t *sys_semCreate(int initial_value) {
  ...
  if (process->sem_group == 0) {
    ...
    if (semaphore_group == 0)
      return (sem_t *)-1; // Luego mirar que error es
  } else {
...
int sys_semWait(sem_t *s) {
  ...
  if (current_semaphore == 0)
    return -1; // Again error, i'll look it up later
  if (current_semaphore->owner_TID == -1)
    return -1;
...
int sys_semSignal(sem_t *s) {
  ...
  if (++(current_semaphore->counter) >= 0 &&
    ...
    struct list_head *first_blocked =
        list_first(&current_semaphore->blocked_anchor);
    if (unblock(&current_semaphore->blocked_anchor) < 0)
      return -1;
